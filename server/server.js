  ...
  try {
    ...
    if (imageUrl.startsWith('data:image/')) {
      ...
      try {
        ...
        console.log(`Successfully fetched and converted image from URL, size: ${imageBuffer.length} bytes`);
      } catch (fetchError) {
      ...
      return res.status(400).json({ 
        error: 'Invalid image URL format. Must be a data URL starting with "data:image/" or an HTTP URL',
